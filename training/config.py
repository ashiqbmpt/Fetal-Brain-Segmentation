...
config['parentdir'] = r'E:\Feta/'                                                       # main directory
...
config['out_channels'] = 8                                                               # number of classes (Use 1 for binary, Use total number of classes with background for multiclass)
config['palette'] = [0, 0, 0, 0, 0, 255, 255, 255, 0, 255, 0, 255, 255, 165, 0, 192, 192, 192, 0, 0, 128, 128, 0, 128]
config['disregard_background'] = True                                                    # For Multi-Class Only : Whether to Disregard Background (0 Class) in metric calculation
config['input_mean'] = [0.0528, 0.0528, 0.0528]                                            # provide 3 numbers for RGB images or 1 number for gray scale images in list format
config['input_std'] = [0.1578, 0.1578, 0.1578]                                             # provide 3 numbers for RGB images or 1 number for gray scale images in list format
...
config['lossType'] = 'CrossEntropy'                                                          # loss function: 'CrossEntropy' for multi-class. 'BCELoss' or 'DiceLoss' for binary class
...
config['model_to_load'] = 'resnet34*Segformer'                                    # enter model name like this: <encoder_name>*<decoder_name> (names should match the documentation exactly)
config['model_name'] = 'resnet34_MLP'                              # choose a unique name for result folder
...
config['Results_path'] = r"E:\Feta/" + "Results"   # main results file
config['save_path'] = config['Results_path'] +'/'+ config['model_name']                  # save path
config['generated_masks']  =  r'E:\Feta/' + 'Generated_Masks'         # path to save generated_masks for test set
##################
